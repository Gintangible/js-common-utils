...
exports["default"] = translateConsoleArguments;
 ...
 */
function translateConsoleArguments(msg, args) {
  if (!args) {
    return [msg];
  } // 根据占位符模式 {0}, {1}, {2} ... 等分隔消息模板


  var result = msg.split(/(\{[0-9]+\})/); // replaced[i] 表示result中第i个元素有没有被替换过

  var replaced = result.map(function () {
    return false;
  }); // 对于每个参数值，用它的值替换result数组中对应的占位符

  args.forEach(function (arg, i) {
    // placeholder为第i个参数对应的占位符字符串'{i}'
    var placeholder = "{".concat(String(i), "}");
